      ...
      <template v-else>
        ...
        <div class="text-center mb-8 sm:mb-10 pt-4">
          ...
          <div class="relative inline-flex items-center justify-center mb-5">
            <div
              class="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-emerald-50 dark:bg-emerald-950/30 border-2 border-emerald-200 dark:border-emerald-800/50 flex items-center justify-center success-pulse">
              <Icon name="lucide:check-circle-2"
                class="w-10 h-10 sm:w-12 sm:h-12 text-emerald-500 dark:text-emerald-400" />
            </div>
          </div>
          <div
            class="inline-flex items-center gap-2 bg-[#F0F0F0] dark:bg-neutral-800 text-gray-600 dark:text-neutral-400 text-xs font-mono px-3.5 py-1.5 rounded-full mb-3">
          ...
          <p class="text-gray-500 dark:text-neutral-400 text-sm sm:text-base max-w-md mx-auto">
            Thank you, <strong class="text-black dark:text-white">{{ order.customer.firstName }}</strong>! Your order
            has been received and is being prepared.
          </p>
          <p class="text-xs text-gray-400 dark:text-neutral-500 mt-2">
            A confirmation will be sent to <span class="font-medium text-black dark:text-white">{{ order.customer.email
              }}</span>
          </p>
        </div>
        ...
        <div
          class="border border-gray-200 dark:border-neutral-800 rounded-[20px] p-5 sm:p-6 bg-white dark:bg-[#1A1D25] mb-6">
          ...
          <div class="relative flex items-start justify-between gap-2">
            ...
            <div class="absolute top-4 start-0 end-0 h-0.5 bg-gray-200 dark:bg-neutral-700 mx-8">
              <div class="h-full bg-emerald-500 transition-all duration-700" :style="{ width: progressWidth }" />
            </div>

            <div v-for="(step, idx) in timelineSteps" :key="step.id"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
          ...
          <!-- Ordered Items Card -->
          <div
            class="md:col-span-2 border border-gray-200 dark:border-neutral-800 rounded-[20px] p-5 sm:p-6 bg-white dark:bg-[#1A1D25] space-y-4">
            ...
            <div class="divide-y divide-gray-100 dark:divide-neutral-800 space-y-1">
              <div v-for="item in order.items" :key="item.id" class="flex items-center gap-4 py-3 first:pt-0 last:pb-0">
                <div
                  class="w-16 h-16 sm:w-20 sm:h-20 rounded-[12px] bg-[#F0EEED] dark:bg-neutral-800 overflow-hidden flex items-center justify-center shrink-0">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="flex flex-wrap gap-1.5 mt-1">
                    <span
                      class="text-xs bg-[#F0F0F0] dark:bg-neutral-800 text-gray-600 dark:text-neutral-400 px-2 py-0.5 rounded-full">{{
                      item.size }}</span>
                    <span
                      class="text-xs bg-[#F0F0F0] dark:bg-neutral-800 text-gray-600 dark:text-neutral-400 px-2 py-0.5 rounded-full">{{
                      item.color }}</span>
                    <span
                      class="text-xs bg-[#F0F0F0] dark:bg-neutral-800 text-gray-600 dark:text-neutral-400 px-2 py-0.5 rounded-full">Qty:
                      {{ item.quantity }}</span>
                  </div>
                </div>
                <div class="text-end shrink-0">
                  <p class="font-bold text-base text-black dark:text-white">${{ item.price * item.quantity }}</p>
                  <p v-if="item.quantity > 1" class="text-xs text-gray-400 dark:text-neutral-500">${{ item.price }} each
                  </p>
                </div>
              </div>
            </div>
          ...
          <div
            ...
            <div class="space-y-2 text-sm">
              <div class="flex gap-2">
                ...
                <span class="text-black dark:text-white font-medium">{{ order.customer.firstName }} {{
          ...
          <div
            ...
            <div class="space-y-3 text-sm">
              ...
              <div v-if="order.payment.cardLast4" class="flex items-center justify-between">
                ...
                <span class="font-mono font-medium text-black dark:text-white">•••• •••• •••• {{ order.payment.cardLast4
                  }}</span>
              </div>
              <!-- Payment Status -->
        ...
        <div class="flex flex-col sm:flex-row gap-3 justify-center mt-8">
          <LazyVButton to="/products" variant="outline" color="dark" shape="pill" size="lg" class="px-8">
            <Icon name="lucide:shopping-bag" class="w-4 h-4" />
            Continue Shopping
          </LazyVButton>
          <LazyVButton @click="printReceipt" variant="solid" color="dark" shape="pill" size="lg" class="px-8">
